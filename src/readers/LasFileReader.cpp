...
std::vector<Lpoint> LasFileReader::read()
{
	...
	// TODO: manage no RGB .las format
	mainOptions.numCols = 15;

	if (!mainOptions.userNumPoints) mainOptions.numPoints = std::numeric_limits<unsigned int>::max();

	// Main bucle
	while (lasreader->read_point() && idx < mainOptions.numPoints)
	{
